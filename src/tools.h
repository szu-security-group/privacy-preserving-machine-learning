...
* This file is part of the secure-nn project, is was taken from the file tools.h written by Sameer Wagh.
*/
...
{	
	...
	if (r_type == "COMMON")
	{
		...
		{		
			...
			// {
			// 	for (size_t i = 0; i < size; ++i)
			// 		vec[i] = sign*aes_common->get32Bits();		
			// }
			else
			{
				for (size_t i = 0; i < size; ++i)
...
void addModuloOdd(const vector<T1> &a, const vector<T2> &b, vector<myType> &c, size_t size)
{
	...
	for (size_t i = 0; i < size; ++i)
	{
		if (a[i] == MINUS_ONE and b[i] == MINUS_ONE)
			...
			c[i] = (a[i] + b[i] + wrapAround(a[i], b[i])) % MINUS_ONE;//负+负+正（带正负号计算）
		// cout << a[i] << " " <<b[i] << " "<< (T1)wrapAround(a[i], b[i]) << " " <<c[i] << endl;
	}
}
