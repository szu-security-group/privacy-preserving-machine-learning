...
* This file is part of the secure-nn project, is was taken from the file connect.cpp written by Sameer Wagh.
*/
...
void initializeCommunicationSerial(int* ports)//Use this for many parties
{
	...
	{//3PC:partyNum=2 (0,1)先发送后接收;
	//partyNum=1 (0)先发送后接收(2)先接收后发送;
	//partyNum=0 (1,2)先接收后发送
		if (i<partyNum)
		{
		  initCommunication( addrs[i], ports[i * 2 + 1], i, 0);//connect发送数据
		  initCommunication("127.0.0.1", ports[i * 2], i, 1);//listen接收数据
		}
		else if (i>partyNum)
		{
...
void initializeCommunication(char* filename, int p)
{
	...
	addrs = new string[NUM_OF_PARTIES];//3PC NUM_OF_PARTIES=3
	...
	for (int i = 0; i < NUM_OF_PARTIES; i++)
	{
		...
		sscanf(buff, "%s\n", ip);
		addrs[i] = string(ip);
		// cout << addrs[i] << endl;//取parties_localhost中的ip地址
...
void synchronize(int length = 1)
{
	...
	for (int i = 0; i < NUM_OF_PARTIES; i++)
	{
		...
		{
			threads.push_back(new thread(sendByte, i, toSend, length, conn));//添加线程
